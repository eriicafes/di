...
export class DIContainer {
  ...
  private storeInstance<T>(
    instanceMeta: InstanceMeta | undefined,
    instance: T
  ) {
    if (instanceMeta) {
  ...
  public registerToken(token: RegisterToken) {
    ...
    if (token.factory.length <= 1) {
      ...
      throw new Error(
        `Invalid factory function provided while registering token ${token.identifier.toString()}`
      );
    }
  }
  ...
  private resolveToken<T>(
    ...
    try {
      ...
      if (!cachedTarget) {
        throw new Error(
          `Unresolved Token "${target.toString()}" in ${this.traceChild(
            childTrace
          ).join(" > ")}`
        );
      }
  ...
  public resolve<T>(target: Target<T> | FactoryTarget<T> | TokenIdentifier): T {
    ...
    if (
      ...
      throw new Error(
        `Cannot resolve built-in constructor "[${target.name}]" in ${this.name}`
      );
    }
    ...
    if (dependencies.length !== target.length) {
      throw new Error(
        `Dependency mismatch, are you sure "${
          target.name || "(anonymous class)"
        }" is injectable?`
      );
    }
